...
MAINTYPE main()
	...
	FILE *values;
    values = fopen("/home/xgx/annot.txt", "wt");
    fclose(values);
	...
	for(recNum = 0; recNum < REC_COUNT; ++recNum)
		{
		sprintf(record,"%d",Records[recNum]) ;  // change int to string by xgx
		printf("Record %d \n",Records[recNum]) ;
		...
		while(NextSample(ecg,2,InputFileSampleFrequency,SAMPLE_RATE,0) >= 0)
			...
			if(delay != 0)
				...
				values = fopen("/home/xgx/annot.txt", "a+");
                fprintf(values, "%ld,%d\n", DetectionTime,beatType);
                fclose(values);

				printf("qrs position %ld\n",DetectionTime);
